...
if isfile('spoofz/games/'..vape.Place..'.lua') then
	loadstring(readfile('spoofz/games/'..vape.Place..'.lua'), 'bridge duel')()
else
	if not shared.SpoofzDeveloper then
		local suc, res = pcall(function() return 
			game:HttpGet('https://raw.githubusercontent.com/thegrateminecra/spoofz/'..readfile('spoofz/profiles/commit.txt')..'/games/'..vape.Place..'.lua', true) 
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('spoofz/games/'..vape.Place..'.lua'), 'bridge duel')()
		end
	end
end