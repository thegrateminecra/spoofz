...
local drawingactor = loadstring(downloadFile('spoofz/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		local oldreload = shared.spoofzreload
		vape.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.spoofz
			local executionString = "loadfile('spoofz/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.spoofzreload = true\n'..executionString
			end
			...
			notif('Spoofz', 'Failed to find actor', 10, 'alert')
		end)
	else
		vape.Load = function()
			notif('Spoofz', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('Spoofz', 'Failed to hook ('..id..')', 10, 'alert')
	end
