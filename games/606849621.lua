local loadstring = function(...)
	...
	if err and vape then vape:CreateNotification('Spoofz', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local vm = loadstring(downloadFile('spoofz/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Spoofz', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
