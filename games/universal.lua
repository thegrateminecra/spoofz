...
local function serverHop(pointer, filter)
	visited = shared.spoofzserverhoplist and shared.spoofzserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Spoofz', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Spoofz', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Spoofz', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Spoofz', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
vape:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.spoofzserverhoplist = '"..table.concat(visited, '/').."'\nshared.spoofzserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('spoofz/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('spoofz/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('spoofz/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				vape.Uninject = function()
					notif('Spoofz', 'No escaping the private members :)', 10)
				end
			end
		end
	end
	...
	function whitelist:update(first)
		local suc = pcall(function()
			...
			return game:HttpGet('https://github.com/thegrateminecra/spoofz')
		end)
		local commit = subbed:find('currentOid')
		commit = commit and subbed:sub(commit + 13, commit + 52) or nil
		commit = commit and #commit == 40 and commit or 'main'
		whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/thegrateminecra/spoofz/main/profiles/whitelist.json', true)
		end)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('spoofz/profiles/whitelist.json') and readfile('spoofz/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('spoofz/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('spoofz/assets/new/arrowmodule.png')
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('spoofz/assets/new/radaricon.png'),
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.spoofzsessioninfo = '"..httpService:JSONEncode(vape.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.spoofzsessioninfo then
					for i, v in httpService:JSONDecode(shared.spoofzsessioninfo) do
...
run(function()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.spoofzserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.spoofzserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.spoofzserverhopprevious)
			end
		end
	})
end)
